 ...
 */
// 发牌动画控制器
GameWindowPinShi.PlayCardsAnimation = cc.Class.extend({
    _node               : null,         // 本节点
    ...
    ctor : function (node) {
        ...
        this._node = ccs.load("res/Games/PinShi/PlayCardAnimation/PlayCardAnimation.json").node;
     ...
     */
    playAnimation : function (roomUI, playerIndexArray, count, completeCallback) {
        this.reset();
        this._roomUI = roomUI;
        this._count = count;
    ...
    _playCardsToPlayer : function () {
        ...
        for (var i = length - 1; i >= 0; --i) {
            ...
            var delay = 0.3;
            for (var j = this._count; j > 0; --j) {
                this._playMoveCard(delay * j, uiPlayer, position, scale);
            }
        }
        ...
        this._node.runAction(cc.sequence(cc.delayTime(1.5), fun));
    },
    ...
    _playMoveCard : function (delay, uiPlayer, position, scale) {
        var sp = new cc.Sprite("res/Games/PinShi/Pokers/100.png");
        ...
        var fun = cc.CallFunc(function () {
            ...
            // 添加牌背的牌
            uiPlayer.getHandCards().addCards([100]);
        });
