...
WindowRun.Player = GameWindowBasic.GamePlayerBasic.extend({
    ...
    ctor : function (node, index) {
        ...
        this._node = ccs.load("res/Games/Run/Players/Player_" + index + ".json").node;
        this._index = index;
    ...
    _init : function () {
        ...
        this._userInfo = new WindowRun.UserInfo(game.findUI(this._node, "ND_UserInfo"), this._index);
        ...
        this._imgSingle = game.findUI(this._node, "ND_BaoDan");
        this._double = game.findUI(this._node,"ND_Double");
        this._doubleNum = game.findUI(this._double, "num");
        ...
        if (this._handCards == null) {
            if (this._handCardsNode) {
                this._handCards = new WindowRun.HandButtonCards(this._handCardsNode);
    ...
    setInfo : function (index, info) {
        ...
        cc.log("==> 跑得快玩家信息：" + index + " " + JSON.stringify(info));
    ...
    // 设置炸弹个数
    setDouble:function(num){
        this._double.setVisible(num > 0);
        this._doubleNum.setString("" + num);
    },
    ...
    showTip: function (tip) {
        ...
        var path = "res/Games/Run/Image/TXT_Tip_" + tip + ".png";
    ...
    setDealer : function (dealerIndex) {
        if (dealerIndex != -1 && dealerIndex == this._logicIndex) {
            this._imgDealer.setVisible(true);
            this._imgDealer.runAction(cc.Sequence(cc.DelayTime(3.0), cc.CallFunc(function () {
     ...
     * 显示报单
     */
    showBaoDan:function (show) {
        this._imgSingle.setVisible(show);
    },
    ...
    getLeftCards : function () {
        if (this._leftCards == null) {
            if (this._leftCardsNode) {
                this._leftCards = new WindowRun.PlayerLeftCards(this._leftCardsNode);
    ...
    getOutCards : function () {
        if (this._outCards == null) {
            if (this._outCardsNode) {
                if(this._index == 1){
                    this._outCards = new WindowRun.HandCards(this._outCardsNode);
                }else if(this._index == 2){
                    this._outCards = new WindowRun.HandCards2(this._outCardsNode);
                }else if(this._index == 3 || this._index == 4){
                    this._outCards = new WindowRun.HandCards3(this._outCardsNode);
                }
