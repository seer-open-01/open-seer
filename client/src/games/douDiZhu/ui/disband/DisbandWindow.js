...
GameWindowDouDiZhu.DouDiZhuDisbandWindow = game.ui.PopupWindow.extend({
    ...
    ctor: function () {
        ...
        this._node = ccs.load("res/Games/DouDiZhu/Disband/Disband.json").node;
        this._super();
    ...
    //外部调用，当有玩家申请解散 或者退出游戏的时候调用
    //requestindex申请者的index
    //当前申请解散的房间里面的玩家数据
    setDate: function (requestindex) {

        var Data = game.procedure.DouDiZhu.getGameData();
        ...
        for (var index in players) {
            if (players.hasOwnProperty(index)) {
                if (players[index]) {
                    var Item = new GameWindowDouDiZhu.playerInfoItem(requestindex, players[index], this._playerUiNode[idx]);
                    //这里要先设置ID 在设置信息，
                    Item._id = +index;
                    Item._setInfo();
    ...
    SetPlayerIsAgree: function (index, flag) {
        for (var i = 0; i < this._playerInfoItem.length; ++i) {
            if (this._playerInfoItem[i]._id == index && this._playerInfoItem[i]._id!=this._playerInfoItem[i].getRequestindex()) {
    ...
    //销毁定时器
    _destroy: function () {
        GameWindowDouDiZhu.DouDiZhuDisbandWindow.inst = null;
        game.UISystem.closePopupWindow(this);
    }
});
//弹出窗口
...
GameWindowDouDiZhu.DouDiZhuDisbandWindow.popup = function (requestindex) {
    var win = new GameWindowDouDiZhu.DouDiZhuDisbandWindow();
    GameWindowDouDiZhu.DouDiZhuDisbandWindow.inst = win;
