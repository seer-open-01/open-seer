 ...
 * @returns {*}
 */
Date.prototype.stdFormatedToday = function() {
 ...
 * @returns {*}
 */
Date.stdFormatedByIV = function(n){
    return (new Date(Date.getStamp() * 1000 + n * 3600 * 24 * 1000)).stdFormatedToday();
}
/**
 *  时间戳 格式化
 */
Date.formatDate = function(stamp)
{
...
Date.zeroTime = function () {
    return Date.prototype.zeroTime();
};
 ...
 * @param date
 */
Date.getStampBySql = function(date)
{
 ...
 * @param stamp
 */
Date.getMysqlDate = function (stamp) {
    var time = this.formatDate(stamp);
    var str = time.year + "-" + time.month + "-" + time.day + " " + time.hour + ":" + time.minute + ":" + time.second;
    return str;
};
