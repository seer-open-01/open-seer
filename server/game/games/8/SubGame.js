...
SubGame.prototype = {
    ...
    initProtoMap: function() {
        this.onProto(ProtoID.CLIENT_GAME_REQ_HSZ,this.onPlayerReqHSZ);                                             //  请求换三张的牌
        this.onProto(ProtoID.CLIENT_GAME_REQ_DQ,this.onPlayerDQ);                                                  //  请求定缺
        ...
        this.onProto(ProtoID.CLIENT_GAME_GET_MISS,this.onPlayerGetHszInfo);                                        //  获取换牌信息2:胡牌提示 3:对局流水
        this.onProto(ProtoID.CLIENT_GAME_REQ_TING,this.onPlayerTing);                                              //  请求听牌
        this.onProto(ProtoID.CLIENT_GAME_PVP_DATA,this.onPlayerPVP);                                               //  请求PVP数据
    },
     ...
     * @param rArgs
     */
    onPlayerReqHSZ(wsConn, rState, rArgs){
        let uid = +rArgs.uid;
        let roomId = +rArgs.roomId;
        ...
        wantRoom.onPlayerReqHSZ(uid, cards);
    },
    /**
     * 请求定缺
     * @param wsConn
     ...
     * @param rArgs
     */
    onPlayerDQ(wsConn, rState, rArgs){
        let uid = +rArgs.uid;
        let roomId = +rArgs.roomId;
        let type = +rArgs.type;
        let wantRoom = this.rooms[roomId];
     ...
     * @param rArgs
     */
    onPlayerGetHszInfo:function(wsConn, rState, rArgs){
        let uid = +rArgs.uid;
        let roomId = +rArgs.roomId;
        let wantRoom = this.rooms[roomId];
        if(!wantRoom){
        ...
        wantRoom.onPlayerGetHszInfo(uid);
    },
    /**
     * 请求听牌
     */
    onPlayerTing(wsConn, rState, rArgs){
        let uid = +rArgs.uid;
        let roomId = +rArgs.roomId;
        let wantRoom = this.rooms[roomId];
        let cards = rArgs.cards;
        ...
        wantRoom.onPlayerTing(uid);
    },

    onPlayerPVP(wsConn, rState, rArgs){
        let uid = +rArgs.uid;
        let roomId = +rArgs.roomId;
        let wantRoom = this.rooms[roomId];
        if(!wantRoom){
