define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function ($, undefined, Backend, Table, Form) {
    ...
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'player/agent/index',
                    del_url: 'player/agent/set',
                    multi_url: 'player/agent/multi',
                    table: 'user_list'
                }
            });
                ...
                columns: [
                    [
                        ...
                        {field: 'uid', title: __('操作'), table: table, buttons: [
                                {name: 'detail', text: '', title: '设置代理', icon: 'fa fa-magic', classname: "btn btn-xs btn-danger btn-delone {:$auth->check('player/agent/set')?'':'hide'}", url: 'player/agent/set', success:function(data, ret){
